...
app.get('/api/cities', async (req, res) => {
  const limit = number(req.query.limit ?? 100, 'limit', 1, 500, true);
  const offset = number(req.query.offset ?? 0, 'offset', 0, 2147483647, true);
  const rows = await db.select().from(cities).orderBy(asc(cities.id)).limit(limit).offset(offset);
...
module.exports = { app, pool, db, city, cityValues };
